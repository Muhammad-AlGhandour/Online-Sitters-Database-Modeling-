-- 
-- Group 100
-- Members: Shedrach Ezenwali, MySQL Expert
--          Al Ghandour Muhammad, MS SQL Expert
-- --------------------------------------

-- Inserting script works on both MS SQL and MYSQL
...
--SELECT child_name, (YEAR(GETDATE()) - YEAR(birth_date)) AS Age
--FROM Child
--HAVING Age = (SELECT  Max((YEAR(GETDATE()) - YEAR(birth_date)))
--FROM Child);
SELECT child_name, (YEAR(GETDATE()) - YEAR(birth_date)) AS Age
FROM Child
WHERE (YEAR(GETDATE()) - YEAR(birth_date)) = (
    SELECT MAX((YEAR(GETDATE()) - YEAR(birth_date)))
    FROM Child
);
...
CREATE VIEW preference_occurrances AS
